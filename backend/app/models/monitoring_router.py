...
@router.post("/{model_id}/observations", response_model=MonitoringObservationRead, status_code=status.HTTP_201_CREATED)
def create_observation(
    ...
    service: MonitoringObservationService = Depends()
):
    # Pass authenticated user. Service enforces that model_id in payload aligns with the URL model_id.
    return service.create_observation(model_id, observation_in, owner_id=current_user.id)

@router.get("/{model_id}/observations", response_model=List[MonitoringObservationRead])
def list_observations(
    ...
    return service.list_observations(model_id, skip=skip, limit=limit, owner_id=current_user.id)

@router.get("/{model_id}/observations/{observation_id}", response_model=MonitoringObservationRead)
def get_observation(
    model_id: uuid.UUID,
    ...
    obs = service.get_observation(observation_id, owner_id=current_user.id)
    from fastapi import HTTPException
    if obs.model_id != model_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Observation not found")
    return obs

@router.delete("/{model_id}/observations/{observation_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_observation(
    ...
    service.delete_observation(observation_id, owner_id=current_user.id)
